...
| `silence/services/MQTTService.py` | Override : `connect_async` + `loop_start` + `reconnect_delay_set` (callbacks posés avant connexion) au lieu du `connect()` bloquant unique de l'upstream. Sans ça, si mosquitto n'est pas prêt au démarrage (reboot de l'hôte : Docker redémarre les conteneurs sans respecter `depends_on`), silence-server tournait sans MQTT — télémétrie non publiée, commandes ignorées — jusqu'à un restart manuel. La boucle paho retente la connexion initiale et les reconnexions indéfiniment (1→30 s) |
...
| `dashboard/index.template.html` | Interface HTML, placeholder `TON_IMEI` dans `var IMEI`. Connexion MQTT en `wss://` auto si la page est servie en HTTPS (reverse-proxy), sinon `ws://`. **Re-render débouncé** : les messages MQTT par champ n'appellent plus `update()` directement mais `scheduleUpdate()` (100 ms) — un burst SYNC = 1 render au lieu de dizaines ; favicon régénéré seulement quand le % entier de SOC change. **Thème unique teal** (les presets aurora/indigo ont été retirés) ; `theme-color` `#0d1f33`. **Deux modes d'affichage** commutés dans l'en-tête (segmented `#mode-toggle` avec `aria-pressed` sur chaque bouton, persistant `localStorage.mode`, défaut **sobre**, raccourci `M`) : sobre = batterie/autonomie/statut/trajets/position, technique = cockpit complet. Les blocs techniques portent la classe **`tech`** (masquée par `body[data-mode="sobre"] .tech{display:none}` — usage stats, santé batterie, moteur, coûts+tarif, `.col-diag`, page mobile diagnostics + son dot) ; un script inline en tête de `<body>` pose `data-mode` avant le rendu (pas de flash) ; en sobre la grille desktop passe à 3 colonnes et `maxPage()` borne le swipe à 3 pages. Desktop : grille **4 colonnes** (`.col-left` contrôles / `.col-mid` batterie-moteur-coûts-trajets / `.col-diag` diagnostics / `.col-right` carte-trajet). Entre 1025 et 1400px en mode technique, `main` passe à 3 pistes mais garde 4 colonnes DOM : `.col-right` bascule en `grid-column:1/-1` (rangée pleine largeur, grille interne 3 colonnes) pour ne pas écraser ses cartes Leaflet dans une piste de 240px ; sans effet en mode sobre (`.col-diag` déjà masquée, 3 colonnes natives). Mobile : **4 pages** swipe — **ordre DOM : `mp-1` batterie/moteur en page d'atterrissage**, puis `mp-0` contrôles, `mp-2` GPS/trajets, `mp-3` diagnostics ; dots (vrais `<button>` + `aria-label` + `aria-current` sur la page active) en `position:fixed` bas d'écran ; le swipe ignore les gestes commencés sur une carte Leaflet et exige `|dx|>|dy|` (pas de changement de page en scrollant/pannant). Diagnostics mobiles via mirroring `m-` (`txt2`, `setTemp`, `renderCellGrid`). **Feedback commandes** : toute commande (`cmd()`/`syncCmd()`) répond par un toast (`#toast`, `role="status"`) — envoyée, « Hors ligne — commande non envoyée » si MQTT déconnecté, ou échec du publish. **Raccourcis clavier** : `S` sync, `M` bascule sobre/technique, `1`-`4` pages mobiles (ignorés dans les inputs). Jargon technique expliqué par des `title` (SOC Astra, Flags BMS, Déséq. mV, NTC, Bus V, Regen., Cons. %/km, Cycles, coûts amortis, colonnes du tableau des trajets). Icônes des tuiles de statut et des lignes de température/énergie en **SVG trait 2** (mêmes tracés que les boutons de commande et titres de cartes) plutôt qu'en emoji, pour l'unité visuelle inter-OS — emoji conservés là où ils portent un ton ou un état (`🛵`, `📦`, `⚡ Charge`, `🔓 Retirée`, `🗺️`). État de connexion du header annoncé (`aria-live="polite"` sur `#hlbl`), tarif électricité relié à son `<input>` via `<label for>`. Pastille « Veille » stylée `.s1` (gris-bleu, distincte d'Éteint). Gris honnête étendu à toute la ligne de statut et à la jauge : vitesse (`--` tant que `d.speed` est null), SOC (`--%` et jauge/arc vides tant que `SOCbatteria`/`BatterySoC` sont inconnus — favicon et `--soc-color` non touchés dans ce cas), odomètre (`--` si `d.odo` absent) et état batterie `g-state` (`--` tant que `charging`/`batteryOut` n'ont jamais été reçus, jamais un faux « ✓ Normal »). **Puissance de charge** : ligne `g-pw`/`m-g-pw` dans `bat-extras`, masquée par défaut, affichée uniquement en charge (`d.charging` vrai, tension et courant connus) — `|tension × courant| / 1000` en kW, pas d'estimation de temps restant (données insuffisantes). Libellés coûts honnêtes : Semaine ⌀ / Mois ⌀ / Total (amortis sur le kilométrage total, `title` explicatif). Boutons de commande = `<button>` accessibles (`aria-label`, focus clavier). État initial : `<body class="predata">` atténue toutes les `.card` tant qu'aucune donnée (retiré au 1er `update()` ou au 1er `renderTripsTable` avec trajets). `prefers-reduced-motion` respecté (pulse/spin/transitions coupés). Sparklines SVG maison (buffer client échantillonné 10s, fenêtre ~10 min = `HIST_MAX` 60 × 10s) pour temp/élec dans `.col-diag` (desktop uniquement). Chaque sparkline porte une légende d'échelle `.spark-cap` : `↕ min–max + unité` (°C / V / A, calculé par `renderSparks`/`renderArea`) et `↔ ~10 min` pour l'axe temps. **Position actuelle** : carte **Leaflet live** (`showPosMap`/`_posMarkers`, `#pos-map`/`#m-pos-map`) — marqueur déplacé à chaque fix GPS, recentrage seulement si le marqueur sort de la vue (l'ancienne iframe OSM restait figée sur le 1er fix). **Toutes les tuiles OSM sont assombries** via filtre CSS `.leaflet-tile` (invert + hue-rotate) pour coller au thème sombre. **Dernier trajet** : tuile unique `.trip-card` (gros chiffre `.trip-card-head` + grille détail **2 colonnes** `#lt-data` : durée/dist/bat/vavg/vmax/eff) — mobile identique (`#m-lt-data`, page 2), peuplé via `txt2` dans `renderLastTrip` ; le « il y a … » (`lt-ago`) est rafraîchi par le même interval que `updateLastUpdate` (`_lastTripTs`). **Trajet en cours** : card desktop `#trip-live-card` (col-right) + **card mobile séparée `#m-trip-live-card` en enfant direct de `<body>`** (`position:fixed` au-dessus des dots) — ⚠️ ne pas la remettre dans `<main>` : `main` est `display:none` sur mobile, un `fixed` à l'intérieur serait invisible. Les valeurs live lisent `lastData.odo`/`lastData.SOCbatteria` dans le callback du timer (pas les arguments figés dans la closure au démarrage du trajet). **Tracé de trajet** : clic sur une ligne du tableau « Trajets récents » → polyline **Leaflet sur fond OSM** (`renderRoute`/`_ensureMap`, `fitBounds` auto, marqueurs départ/arrivée) dans la carte « Tracé du trajet » (col-right desktop / page 2 mobile, message « Pas de points GPS » si `path` vide — cas des trajets anciens dont le relay a purgé le tracé) ; la ligne cliquée reçoit la classe `.sel` (fond teal) ; chaque ligne porte un `aria-label` nommant l'action (« Afficher le tracé du trajet du … »). Tableau « Trajets récents » : **table plate 7 colonnes** Date / Durée / Dist. / Vmax/Moy (km/h) / Bat. (conso %, `--` si `bat` null) / Effic. (Wh/km) / T° A/O/M (°C, `--` si absent) ; colonne T° masquée sous 400px ; en-têtes alignés à droite (sauf 1re colonne). Tous les trajets rendus dans `.trips-scroll` (hauteur ~5 lignes, en-tête `sticky`). Le tableau est refetché à chaque reconnexion MQTT **et** par un poll 60 s (`fetchTrips`). Durées formatées `fmtDur` (`75min` → `1h15`). Heures formatées **côté client depuis `t.ts`** (`formatTripDate`, locale navigateur) |
