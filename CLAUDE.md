...
| `mqtt-retain/relay.py` | Relais MQTT (service `mqtt-retain`) : republie chaque message en `retain` (snapshot au chargement du dashboard) **et** détecte/persiste les trajets dans `trips.json` (volume `trip-data`, monté `:ro` sur `…/html/data` côté dashboard). Machine à états : trajet démarre quand `status`→4, finit au retour. Enregistre date, durée, distance (Δodo), batterie (Δsoc), vmax, vavg, **et le tracé GPS** (`path` = liste `[lat,lon]` échantillonnée toutes les 3 s via `latitude`/`longitude`, proto Z). ⚠️ Tout `_track_field` doit recevoir `now_ms` en argument (ne pas s'appuyer sur une variable globale — bug historique qui faisait planter le thread et laissait `trips.json` vide) |
| `dashboard/index.template.html` | Interface HTML, placeholder `TON_IMEI` dans `var IMEI`. Desktop : grille **4 colonnes** (`.col-left` contrôles / `.col-mid` batterie-moteur-coûts-trajets / `.col-diag` diagnostics **toujours affichés** / `.col-right` carte-trajet). Mobile : **4 pages** swipe (`mp-0`..`mp-3`, la 4e = diagnostics), dots en `position:fixed` bas d'écran. Diagnostics mobiles via mirroring `m-` (`txt2`, `setTemp`, `renderCellGrid`). Sélecteur de thème (aurora/indigo/teal) persistant via `localStorage`. Sparklines SVG maison (buffer client échantillonné 10s) pour temp/élec dans `.col-diag` (desktop uniquement). **Tracé de trajet** : clic sur une ligne du tableau « Trajets récents » → polyline SVG maison (`renderRoute`, projection longitude×cos(lat), sans dépendance carto) dessinée dans la carte « Tracé du trajet » sous le détail (col-right desktop / page 2 mobile) à partir du `path` GPS du trajet |
